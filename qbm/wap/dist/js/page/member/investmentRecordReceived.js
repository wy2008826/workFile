		...
		require.async('dropload', function(){
			$('#pageWrap').dropload({
		        ...
		        loadUpFn : function(me){
		            $.ajax({
				        type:"get",
				        url:wapurl+"/api/member/getMyInvestmentForAllList.html?type=1&currentPage=1&pageSize=4&randomTime="+(new Date()).getTime(),
				        dataType:"jsonp",
		        ...
		        loadDownFn : function(me){
		        	$.ajax({
				        type:"get",
				        url:wapurl+"/api/member/getMyInvestmentForAllList.html?type=1&currentPage="+page+"&pageSize=4&randomTime="+(new Date()).getTime(),
				        dataType:"jsonp",
				        success:function(data){
				        	$("#totalInterrest").text((data.totalMoney).toFixed(2));
				        	if(data.code == 1){//已登录
