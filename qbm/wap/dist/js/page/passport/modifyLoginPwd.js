define(function(require,module,exports){
	...
	$(function(){
		var App=function(){
			...
			this.url={
				wapUrl:$("#wapurl").val(),
				submitUrl:"/api/passport/security.html"//修改登录密码
			};

			this.alertMsg={
				originIsError:"<p class='text-center lh-40'>原登录密码输入不正确</p>",
				passNotRight:"<p class='text-center lh-40'>抱歉，您输入的登录密码格式不正确，请输入6-12位数字加字母组合，区分大小写</p>",
				passIsNotEqual:"<p class='text-center lh-40'>两次输入的新登录密码不一致</p>"
			};
						...
						$.ajax({
							...
							success:function(data){
								...
								if(!(data.result)){//各种原因的修改失败
									...
									else if(data.code==3){//两次新登录密码不一致
										var html=self.alertMsg.passIsNotEqual;
										self.alertMes(html);
									}
									else if(data.code==4){//新登录密码格式不正确
										var html=self.alertMsg.passNotRight;
										self.alertMes(html);
									}
