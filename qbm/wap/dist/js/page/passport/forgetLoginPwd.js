	...
	$(function(){
		var App=function(){
			...
			this.url={
				...
				submitUrl:"/api/refundPassword.html"//忘记登录密码
			};
			...
			this.alertMsg={
				...
				passNotRight:"<p class='text-center lh-40'>抱歉，您输入的登录密码格式不正确，请输入6-12位数字加字母组合，区分大小写</p>",
				passIsNotEqual:"<p class='text-center lh-40'>两次输入的登录密码不一致</p>"
			};
			...
			this.pagePhone=this.getParam("phoneNumber");//获取当前页面的手机
		...
		App.prototype={
			...
			validateForm:function(){
				...
				function validateAll(){
					var phoneVal=$phone.val().trim();//手机号码
					...
					if( (phoneVal!="")&&(imgVal!="")&&(telMesVal!="")&&(pswVal0!="")&&(pswVal1!="")){//必填项不能为空
						if(!(telReg.test(phoneVal))){//手机号码格式不正确
							return false;
						}
				...
				$submit.click(function(){
					...
					if(!( $submit.hasClass("disabled") ) ){//按钮可点  
						...
						var params={
							phoneNumber:self.pagePhone,
							picCheckCode:$imgInput.val(),//图形码
							phoneCheckCode:$telMesInput.val(),//短信码
						...
						$.ajax({
							...
							success:function(data){
								...
								if(!(data.result)){//各种验证错误导致的注册失败
									if(data.status==12){//图像验证码不正确
										var html=self.alertMsg.imgIsError;
									}
									else if(data.status==13){//验证码不正确
				...
				function submitSuccess(){
					var html="<p class='text-center'>登录密码修改成功</p>";
					layer.open({
						content:html,
						shadeClose:false,
						className:"layer-tip",
						time:2,
							...
							var href=self.url.wapUrl+"/passport/enterPwd.html?phoneNumber="+self.pagePhone;
